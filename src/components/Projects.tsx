...
import { ArrowUp, Car, Spade, Heart } from 'lucide-react';
      ...
      return (
        <div className="relative">
          ...
          <div className="relative p-8 flex items-center justify-center">
            {/* First card (Spade) - slightly rotated and positioned */}
            <div className="absolute -rotate-12 transform translate-x-[-15px] translate-y-[-10px]">
              <div className="bg-white/90 backdrop-blur-sm rounded-lg p-4 shadow-xl border border-white/30">
                <Spade size={32} className="text-black" />
              </div>
            </div>
            
            {/* Second card (Heart) - slightly rotated the other way */}
            <div className="absolute rotate-12 transform translate-x-[15px] translate-y-[10px]">
              <div className="bg-white/90 backdrop-blur-sm rounded-lg p-4 shadow-xl border border-white/30">
      ...
      <div className="container mx-auto px-6">
        ...
        <div className="grid md:grid-cols-1 lg:grid-cols-1 gap-12 max-w-4xl mx-auto">
          {projects.map((project, index) => (
            <Card key={project.title} className="bg-white/10 backdrop-blur-lg border-white/20 hover:bg-white/15 transition-all duration-300 animate-slide-up overflow-hidden group" style={{
                      ...
                      <div className="mb-6">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {project.technologies.map(tech => (
                            <span key={tech} className="px-3 py-1 bg-white/20 text-white text-sm rounded-full backdrop-blur-sm">
                              {tech}
                            </span>
                          ))}
                        </div>
